...
namespace PirateShootingGame
{
    internal class ObjResourceReader
    {
        public static unsafe GlObject CreateFromObjFile(GL Gl, string path, float[] color)
        {
            ...
            List<float[]> faceColors = new(); // Store colors per face

            string currentMaterial = "";
            Dictionary<string, float[]> materialColors = new()
            {
                {"default", color},
                {"Trank_bark", new float[] {0.4f, 0.2f, 0.1f, 1f}}, // Brown bark
                {"polySurface1SG1", new float[] {0.1f, 0.5f, 0.1f, 1f}}, // Green leaves
                {"14052PirateShipmateMuscular_cloth", new float[] {0.6f, 0.3f, 0.1f, 1f}}, // Brown cloth  
                {"14052PirateShipmateMuscular_body", new float[] {0.9f, 0.7f, 0.5f, 1f}}, // Skin tone
                {"14053_Pirate_Shipmate_Old", new float[] {0.8f, 0.6f, 0.4f, 1f}}, // Main character - from your MTL file
                // Add alternative names just in case
                    ...
                    switch (parts[0])
                    {
                        ...
                        case "usemtl":
                            if (parts.Length >= 2)
                            {
                                ...
                                // Skip ground materials entirely
                        ...
                        case "f":
                            if (parts.Length >= 4)
                            {
                                // Skip faces if we're using a ground material
                                if (currentMaterial.ToLower().Contains("ground") || 
                                    currentMaterial.ToLower().Contains("plane") ||
                                    currentMaterial.ToLower().Contains("floor"))
                                {
                                ...
                                // Get color for current material
                                ...
                                for (int i = 1; i < parts.Length; i++)
                                {
                                    if (parts[i].Contains("/"))
                                    {
                                        // Format: v/vt/vn or v/vt - extract just vertex index
                                ...
                                if (vertexIndices.Count >= 3)
                                {
                                    for (int i = 1; i < vertexIndices.Count - 1; i++)
                                    {
                                        ...
                                        faceColors.Add(faceColor); // Store color for this face
                                    }
                                }
                            }
                            break;
                        ...
                        case "g":
                        case "s":
                            // Skip these lines - they're material/grouping related
                            break;
                    }
                }
            }

...
namespace Lab3_3
{
    internal static class Program
    {
        ...
        private static unsafe void SetViewMatrix()
        {
            ...
            var viewMatrix = Matrix4X4.CreateLookAt<float>(
                cameraPos,
                cameraTarget,
                cameraUp
            );
        ...
        private static void UpdateSliceRotation()
        {
            ...
            float delta = rotationSpeed * (float)window.Time;
        ...
        private static System.Numerics.Vector3 ToNumericsVector3(ref Vector3D<float> vec)
        {
            return new System.Numerics.Vector3(vec.X, vec.Y, vec.Z);
        }

        private static void FromNumericsVector3(System.Numerics.Vector3 numericsVec, ref Vector3D<float> vec)
        {
            vec.X = numericsVec.X;
            vec.Y = numericsVec.Y;
            vec.Z = numericsVec.Z;
        ...
        private static unsafe void OnRenderFrame(double deltaTime)
        {
            ...
            SetUniform3(ViewPositionVariableName, cameraDescriptor.Position);
            ...
            var lightColorVec3 = ToNumericsVector3(ref lightColor);
            var lightPositionVec3 = ToNumericsVector3(ref lightPosition);
            var ambientStrengthVec3 = ToNumericsVector3(ref ambientStrength);
            var diffuseStrengthVec3 = ToNumericsVector3(ref diffuseStrength);
            var specularStrengthVec3 = ToNumericsVector3(ref specularStrength);
            var backgroundColorVec3 = ToNumericsVector3(ref backgroundColor);

            ImGui.ColorEdit3("Light Color", ref lightColorVec3);
            ImGui.DragFloat3("Light Position", ref lightPositionVec3, 0.1f);
            ImGui.SliderFloat("Shininess", ref shininess, 1.0f, 128.0f);
            ImGui.SliderFloat3("Ambient Strength", ref ambientStrengthVec3, 0.0f, 1.0f);
            ImGui.SliderFloat3("Diffuse Strength", ref diffuseStrengthVec3, 0.0f, 1.0f);
            ImGui.SliderFloat3("Specular Strength", ref specularStrengthVec3, 0.0f, 1.0f);
            ImGui.ColorEdit3("Background Color", ref backgroundColorVec3);
            
            FromNumericsVector3(lightColorVec3, ref lightColor);
            FromNumericsVector3(lightPositionVec3, ref lightPosition);
            FromNumericsVector3(ambientStrengthVec3, ref ambientStrength);
            FromNumericsVector3(diffuseStrengthVec3, ref diffuseStrength);
            FromNumericsVector3(specularStrengthVec3, ref specularStrength);
            FromNumericsVector3(backgroundColorVec3, ref backgroundColor);
        ...
        private static unsafe void SetUniform3(string uniformName, Vector3D<float> uniformValue)
        {
            int location = Gl.GetUniformLocation(program, uniformName);
            if (location == -1)
            {
        ...
        private static void SetUpRubikCube()
        {
            float spacing = 1.1f;

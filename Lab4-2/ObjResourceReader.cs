    ...
    internal class ObjResourceReader
    {
        ...
        private static unsafe void CreateGlArraysFromObjArrays(float[] faceColor, List<float[]> objVertices, List<int[]> objFaces, List<(int v, int vn)[]> objFacesWithNormals, List<float[]> objNormals, List<float> glVertices, List<float> glColors, List<uint> glIndices)
        {
            ...
            else
            {
                ...
                {
                    var a = new Vector3D<float>(objVertices[face[0] - 1][0], objVertices[face[0] - 1][1], objVertices[face[0] - 1][2]);
                    var b = new Vector3D<float>(objVertices[face[1] - 1][0], objVertices[face[1] - 1][1], objVertices[face[1] - 1][2]);
                    var c = new Vector3D<float>(objVertices[face[2] - 1][0], objVertices[face[2] - 1][1], objVertices[face[2] - 1][2]);
        ...
        private static unsafe void ReadObjDataForTeapot(out List<float[]> objVertices, out List<int[]> objFaces, out List<float[]> objNormals, out List<(int v, int vn)[]> objFacesWithNormals)
        {
            ...
            var stream = typeof(ObjResourceReader).Assembly.GetManifestResourceStream("Szeminarium1_24_02_17_2.Resources.teapot.obj");
            if (stream == null)
            {
                Console.WriteLine("Error: Could not load teapot resource.");
                return;
            }
